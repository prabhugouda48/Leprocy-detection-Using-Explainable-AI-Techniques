...
def history():
    ...
    return render_template('history.html', results=results, images={img.id: img for img in user_images})
...
@app.route('/samples')
def samples():
    # Create static/samples directories if they don't exist
    ...
    irrelevant_dir = os.path.join(samples_dir, 'irrelevant')
    ...
    if len(os.listdir(positive_dir)) == 0:
        for i, img_path in enumerate(glob.glob(os.path.join(dataset_dir, 'positive', '*.*'))):
            if i >= 10:  # Limit to 10 images
                break
            dest = os.path.join(positive_dir, os.path.basename(img_path))
            shutil.copy(img_path, dest)
    
    # Copy negative images if static folder is empty
    if len(os.listdir(negative_dir)) == 0:
        for i, img_path in enumerate(glob.glob(os.path.join(dataset_dir, 'negative', '*.*'))):
            if i >= 10:  # Limit to 10 images
                break
            dest = os.path.join(negative_dir, os.path.basename(img_path))
            shutil.copy(img_path, dest)
    
    # Copy irrelevant images if static folder is empty
    if len(os.listdir(irrelevant_dir)) == 0:
        for i, img_path in enumerate(glob.glob(os.path.join(dataset_dir, 'irrelevant', '*.*'))):
            if i >= 10:  # Limit to 10 images
                break
            dest = os.path.join(irrelevant_dir, os.path.basename(img_path))
            shutil.copy(img_path, dest)
    
    # Get list of sample images for each category
    ...
    irrelevant_images = [f for f in os.listdir(irrelevant_dir) if os.path.isfile(os.path.join(irrelevant_dir, f))]
...
def upload_sample(category, filename):
    ...
    # Construct the sample image path
    sample_path = os.path.join('static', 'samples', category, filename)
